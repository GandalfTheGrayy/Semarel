...
func _test_determinism_rng_isolation_and_first_tick() -> void:
	...
	for index in range(37):
		randi()
	_run_ticks(world, first, 1, 20)
	seed(902)
	for index in range(83):
		randf()
	_run_ticks(world, second, 1, 20)
	...
	PrototypeEntityMovement.new().step(world, clock_store, clock.get_tick_index())
	PrototypeEntityMovement.new().step(world, direct_store, 1)
...
func _test_prototype_terrain_rule_and_blocked_stay() -> void:
	for passable_terrain in [TerrainTypes.Id.LAND, TerrainTypes.Id.SAND, TerrainTypes.Id.ROCK]:
		...
		var moved := PrototypeEntityMovement.new().step(coast_world, coast_store, 17)
	...
	_expect_equal(PrototypeEntityMovement.new().step(water_world, water_store, 17), 0, "WATER blocks every cardinal destination")
	...
	_expect_equal(PrototypeEntityMovement.new().step(blocked_world, blocked_store, 1), 0, "fully blocked entity reports no move")
...
func _test_bounds_one_cell_and_identity_contract() -> void:
	...
	for tick_index in range(1, 21):
		...
		movement.step(world, store, tick_index)
...
func _test_dense_order_independence() -> void:
	...
	var movement := PrototypeEntityMovement.new()
	movement.step(world, first, 23)
	movement.step(world, second, 23)
...
func _test_presentation_snapshot_and_catchup_refresh() -> void:
	...
	while clock.consume_tick():
		frame_moved_count += movement.step(world, store, clock.get_tick_index())
...
func _run_render_schedule(frame_count: int, frame_delta: float) -> Dictionary:
	...
	for frame_index in range(frame_count):
		...
		while clock.consume_tick():
			movement.step(world, store, clock.get_tick_index())
	return {
...
func _run_ticks(world: WorldGrid, store: EntityStore, first_tick: int, tick_count: int) -> void:
	var movement := PrototypeEntityMovement.new()
	for tick_offset in range(tick_count):
		movement.step(world, store, first_tick + tick_offset)
